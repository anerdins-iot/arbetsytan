#!/usr/bin/env bash
# Run file detail modal E2E test (mobile viewport).
# Requires: server on http://localhost:3000, DB seeded (admin@example.com / password123).
# Use PLAYWRIGHT_BROWSERS_PATH if your image has a different browser version than Playwright expects.
set -e
cd "$(dirname "$0")/.."
ROOT="$(pwd)"
...
npx playwright test e2e/fas-04-files-mobile.spec.ts --project=mobile
exit $?
